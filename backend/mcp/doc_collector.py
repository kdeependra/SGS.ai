...
def _detect_doc_type(ext: str, content: str) -> str:
    """Return a human-readable document type."""
    mapping = {
...
def _extract_html_tables(content: str) -> list[dict]:
    ...
    for block in table_blocks:
        # Extract header cells from <th> or first <tr>
        ...
        # If no <th> found, use first data row as headers
...
def _extract_json_tables(content: str) -> list[dict]:
    """Extract tables from JSON arrays of objects."""
    ...
    def _process_array(arr: list, name: str):
        ...
        for obj in arr:
            ...
            for k, v in obj.items():
                all_keys.setdefault(k, []).append(str(v) if v is not None else "")
        ...
        for k, vals in all_keys.items():
            columns.append({
                ...
                "nullable": any(v == "" or v == "None" for v in vals),
            })
        if columns:
            tables.append({
                "table": name,
    ...
    if isinstance(parsed, list):
        _process_array(parsed, "data")
    elif isinstance(parsed, dict):
        # Check top-level keys for arrays of objects
        for key, val in parsed.items():
            if isinstance(val, list) and len(val) > 0 and isinstance(val[0], dict):
                _process_array(val, key)

    return tables


def _extract_yaml_tables(content: str) -> list[dict]:
    """Extract tables from YAML lists of dicts."""
    try:
        import yaml
    except ImportError:
        return []
    ...
    def _process_array(arr: list, name: str):
        ...
        if columns:
            tables.append({"table": name, "row_count": len(arr), "columns": columns})
...
def _extract_pdf_tables_by_position(pdf) -> list[dict]:
    ...
    import re as _re
    import logging
    _log = logging.getLogger("doc_collector")
    ...
    for page_idx, page in enumerate(pdf.pages):
        ...
        # Words with a large x-gap (>12px) get tab-separated (column boundary);
        # smaller gaps get space-joined (same cell, e.g. "Gizmo D").
        tab_lines: list[str] = []
        ...
        _log.warning("Page %d: %d visual lines. First 5: %s",
                     page_idx, len(tab_lines),
                     [l[:80] for l in tab_lines[:5]])

        # Adaptive table detection: try merge_factor 1,2,3.
        # merge_factor=2 handles the common case where 10 columns wrap to 2 lines.
...
def _detect_tables_adaptive(tab_lines: list[str]) -> list[dict]:
    ...
    for start in range(len(tab_lines)):
        ...
        # Try merge_factor = 1, 2, 3 (lines per record)
        for mf in (1, 2, 3):
            ...
            if len(rows) >= 3:
                ...
                # Prefer the merge factor that yields the most rows
...
def _extract_tables(content: str, doc_type: str) -> list[dict]:
    ...
    if doc_type == "markdown":
        ...
        return _extract_json_tables(content)
    elif doc_type in ("yaml",):
        return _extract_yaml_tables(content)
...
def _extract_structure(content: str, doc_type: str) -> dict:
    """Extract structural hints depending on the document type."""
    structure: dict = {}
    if doc_type == "markdown":
    ...
    elif doc_type == "xml" or doc_type == "html":
...
def collect_document_metadata(file_path: str | None = None, content: str | None = None,
    ...
    binary_tables: list[dict] = []  # tables extracted from binary formats
    doc_properties: dict = {}  # document-level properties from binary formats
    ...
    if extracted_tables:
        ...
        # Set column_count / row_count from largest table
